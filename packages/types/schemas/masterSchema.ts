...
export const StableId = z
  ...
  .describe(
    "Kebab-case identifier. Must be unique across all IDs in document.",
  );
...
export const ActiveMysterySchema = z.object({
  ...
  readerKnowledge: z.object({
    confirmed: z.array(z.string()).describe("Facts revealed to reader through any POV"),
    implied: z.array(z.string()).describe("Things reader can reasonably infer"),
  }),

  characterKnowledge: z
    .array(
      z.object({
        heldBy: z.string().describe("Character ID that holds this theory."),
        theory: z
          .string()
        ...
        unaware: z.array(z.string()).describe("Key facts this character doesn't know yet"),
        knows: z.array(z.string()).describe("Key facts this character knows about this mystery"),
      }),
    )
    .describe(
      ...
      significantDates: z
        .array(
          z.object({
            ...
            event: z.string(),
            chapter: z.number().optional().describe("Chapter where this event occurred if occured in the story"),
            importance: z.enum(["Critical", "Major", "Notable", "Minor"]),
          }),
        )
        .optional()
        .describe("Important events anchored in timeline. Can also include events that happened before the story started."),
...
export const CharacterAnalysisSchema = z.object({
  ...
  eroticProfile: z.object({
    ...
    eroticSignificance: z
      .string()
      .describe(
        "How this character's sexual behavior serves the plot. For supernatural stories: magical mechanics tied to sex. For contemporary: how sex reveals character or advances relationships. For mystery: how erotic encounters provide information or create vulnerability. Must be concrete and specific to this story's genre.",
      ),
    ...
    sexualHistory: z
      .array(
        z.object({
          eroticEncounterId: z.string().describe("ID of the erotic encounter"),
          similarEncounters: z.array(z.string()).optional().describe("Other erotic encounters that are similar and should be made into a EroticEncounterTemplate."),
        })
      )
      .optional()
      .describe(
...
export const MasterStoryDocumentSchema = z.object({
  ...
  globalPlotState: z.object({
    ...
    tensionLevel: z
      ...
      .max(10)
      .describe(
        "Current narrative tension on 0-10 scale. Use these anchors: 0-2 = Slice of life, established normalcy. 3-4 = Active questions but no pressure. 5-6 = Protagonist facing challenges with stakes. 7-8 = Multiple active threats or time pressure. 9-10 = Climax-level danger or irreversible decisions imminent. Should generally trend upward until climax."
      ),
  ...
  validationMetadata: z.object({
    ...
    allMysteryIds: z.array(z.string()),
    allTriggerIds: z.array(z.string()),
